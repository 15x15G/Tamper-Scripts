...
// ARIA2地址
let ARIA2_RPC = "http://127.0.0.1:16800/jsonrpc";
...
// 在搜索列表页展示下载进度
const USE_LIST_TASK_STATUS = false;

// 在画廊详情页展示下载进度
const USE_GALLERY_DETAIL_TASK_STATUS = false;

// 在档案下载页面展示下载进度
const USE_HATH_ARCHIVE_TASK_STATUS = false;

// 在种子下载页面展示下载进度
const USE_TORRENT_TASK_STATUS = false;

// 详情页种子下载弹窗显示
const USE_TORRENT_POP_LIST = false;
/**
 * 分类 category
 * 同人志   Doujinshi
 * 漫画     Manga
...
function oneClickButton(gid, pageLink, archiverLink) {
    ...
    oneClick.onclick = async () => {
        ...
        try {
            ...
            // aaa.href = downloadLink;
            // aaa.setAttribute('download',true);
            // aaa.setAttribute('target','_parent');
            // document.body.appendChild(aaa);
            // aaa.dispatchEvent(new MouseEvent("click", {ctrlKey: true}));
            //aaa.click();
            popUp(downloadLink,480,320)
